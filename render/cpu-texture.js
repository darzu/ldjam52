...
export function createTextureReader(data, size, outArity, format) {
    ...
    function read(out, x, y) {
        ...
        if (outArity === 1) {
            ...
            return vec2.set(f32[idx], f32[idx + 1], out);
        }
        else if (outArity === 3) {
            return vec3.set(f32[idx], f32[idx + 1], f32[idx + 2], out);
        }
        else if (outArity === 4) {
            return vec4.set(f32[idx + 0], f32[idx + 1], f32[idx + 2], f32[idx + 3], out);
        }
        else {
            never(outArity);
        }
    ...
    function sample(out, x, y) {
        ...
        if (outArity === 1) {
            ...
            return vec2.set(_sample(0), _sample(1), out);
        }
        else if (outArity === 3) {
            return vec3.set(_sample(0), _sample(1), _sample(2), out);
        }
        else if (outArity === 4) {
            return vec4.set(_sample(0), _sample(1), _sample(2), _sample(3), out);
        }
        else {
            never(outArity);
        }
