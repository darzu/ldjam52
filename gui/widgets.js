...
async function initDragBox() {
    ...
    EM.registerSystem(null, [MouseDragDef, CameraViewDef, WidgetLayerDef], (_, { mousedrag, cameraView, widgets }) => {
        // update dragbox
        if (widgets.cursor || mousedrag.isDragEnd) {
            ...
            // place dragbox
            const min = screenPosToWorldPos(tempVec3(), mousedrag.dragMin, cameraView);
            min[1] = 0;
            const max = screenPosToWorldPos(tempVec3(), mousedrag.dragMax, cameraView);
            max[1] = 1;
...
async function initWidgets({ assets }) {
    ...
    EM.registerSystem(null, [
        ...
        CameraViewDef,
        UICursorDef,
    ], (_, { widgets, physicsResults, mousedrag, cameraView, uiCursor: { cursor: { id: cursorId }, }, }) => {
        const { selected, hover, moved } = widgets;
        moved.clear();
        // update world drag
        let worldDrag = vec3.create();
        if (mousedrag.isDragging) {
            const start = screenPosToWorldPos(tempVec3(), mousedrag.dragLastEnd, cameraView);
            start[1] = 0;
            const end = screenPosToWorldPos(tempVec3(), mousedrag.dragEnd, cameraView);
            end[1] = 0;
