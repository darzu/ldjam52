...
export function createHomeShip() {
    ...
    if (false)
        for (let ccwi = 0; ccwi < 2; ccwi++) {
            ...
            const ccwf = ccw ? -1 : 1;
            let xFactor = 0.05;
            const wallOffset = V(-ribWidth, 0, ribDepth * -ccwf);
            ...
            for (let i = 0; i < numRibSegs; i++) {
                ...
                mat4.copy(builder.cursor, cursor2);
                mat4.translate(builder.cursor, [0, 1, 0], builder.cursor);
                mat4.rotateX(builder.cursor, Math.PI * xFactor * 1.0 * ccwf, builder.cursor);
                mat4.translate(builder.cursor, wallOffset, builder.cursor);
...
function mirrorPath(p, planeNorm) {
    ...
    let mirrorMat3 = mat3.set(1 - 2 * a ** 2, -2 * a * b, -2 * a * c, -2 * a * b, 1 - 2 * b ** 2, -2 * b * c, -2 * a * c, -2 * b * c, 1 - 2 * c ** 2);
